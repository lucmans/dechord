...
To test the monophonic performance of our system, we first measure the performance of scales at different pitch ranges. We expect the system to perform better at higher pitches, as the distance in frequency between is higher. The scales are played at a speed of two notes per second. This equates to playing fourth notes on 120 BPM. We play the scales at a relatively slow speed, such that this cannot become a limiting factor. The slower speed also helps to see if mistakes are the result of transients, as these mistakes should vanish as the note persists. We only play the C major scale, but change the starting note (mode). We play the full vertical scale according to this position~\cite{scale}. Each position has 17 notes, except for $\text{F}_2$, which has 18 notes. In Table~\ref{tab:monperf}, we summarize the results of the experiment. We denote the different starting positions as $n_\text{start}$. For every starting position, we denote the accuracy ($a$), the accuracy without transient errors ($a_\text{trans}$), the accuracy without octave errors $a_\text{oct}$ and the total accuracy without both of these errors ($a_\text{tot}$). The accuracy is calculated by dividing the number of correctly determined frames by the total number of frames with notes. The only frames without notes are positioned at the start and end of a file. We omitted these frames from the results, as they are always correctly determined to be empty frames.
\begin{table}[H]\vspace{-3mm}
    \centering
    ...
    \label{tab:monperf}
\end{table}\vspace{-3mm}
As mentioned before, our system is not yet resilient against noise from transients. Also, solving the octave problem is outside the scope of this paper. Because of this, we also list the results ignoring these problems. TODO: reflect on results

\subsection{Polyphonic performance: intervals}
To test the polyphonic performance of our system, we let it determine the played notes in an interval. We will play every interval within an octave to test if there are any problems with separating the notes in specific intervals. For instance, some interference may cause oscillating peaks in the frequency domain. Again, we will try all the intervals at different root notes, as especially for intervals the pitch of the root note may make a big difference. Even though the overtones of low pitched notes in small intervals might make it possible to distinguish the individual notes, we might not detect enough overtones to actually do so. In Table~\ref{tab:polperf} is a summary of the results. Here, we list the number of semitones $n$ between the two notes in an interval, if both notes were correctly found (yes or no) and if there were other notes reported which were not transients.
\begin{table}[H]\vspace{-3mm}
    \centering
    \begin{tabular}{r|rr}
        $n$ & both & other \\
